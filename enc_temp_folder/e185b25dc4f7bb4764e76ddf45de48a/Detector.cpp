...
void processing(cv::Mat src) {
    ...
    /*
    input = input[0].permute({ 1, 2, 0 });
    input = input * 255;

    std::string a = std::to_string(input.sizes()[0]).append(std::to_string(input.sizes()[1])).append(std::to_string(input.sizes()[2]));
    std::wstring b;
    b.assign(a.begin(), a.end());
    OutputDebugString(b.c_str());
    

    cv::Mat mat = cv::Mat(800, 800, CV_8UC3, input2.data_ptr());
    cv::imshow("image", mat);
    cv::waitKey(0);
    */
    try {
